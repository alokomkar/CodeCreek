package com.sortedqueue.programmercreek.constants;

public class ProgrammingBuddyConstants {
	public static final String KEY_PROG_ID = "Program_Index";
	public static final String KEY_MODULE_LIST = "Modules_List";
	public static final int KEY_TEST = 1;
	public static final int KEY_LIST = 2;
	public static final int KEY_WIZARD = 3;
	public static final int KEY_MATCH = 4;
	public static final int KEY_QUIZ = 5;
	public static final int KEY_REVISE = 6;
	public static String KEY_PROG_TABLE_INSERT = "insertProgramTable";
	public static String KEY_PROG_INDEX_INSERT = "insertProgramIndex";
	public static String KEY_QUIZ_TYPE = "Quiz_Type";

	public static final int KEY_QUIZ_DESCRIPTION_QUESTION = 1;
	public static final int KEY_QUIZ_PROGRAM_CODE_QUESTION = 0;
	public static final String KEY_WIZARD_MODE = "wizard_mode";
	public static String KEY_INVOKE_TEST = "invoke_test";


}
